...
public class GoEffectController<T> : MonoBehaviour where T : Renderer
{
    ...
    /// </summary>
    /// <param name="Render">Graphic及其子类</param>
    /// <param name="fade">新建的渐变方法</param>
    /// <param name="endHander">每开始一次循环进行执行的函数</param>
    /// <returns>这个渐变方法的实时状态</returns>
    public FadeEffectGO<T> StartFade(T Render, FadeEffectGO<T> fade)
    {
        FadeEffectGO<T> copyFade = fade.Start(Render);
    ...
    protected virtual void FixedUpdateNew()
    {
